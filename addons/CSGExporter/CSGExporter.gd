...
func _enter_tree():
	exporter_button = exporter_button_scene.instantiate()
	add_control_to_container(EditorPlugin.CONTAINER_SPATIAL_EDITOR_MENU, exporter_button)
	exporter_button.hide()
	exporter_button.connect("finished_exporting", _on_finished_exporting)

func _handles(object):
	var is_exportable_csg = object is CSGShape3D and object.is_root_shape()
...
func _exit_tree():
	if (exporter_button):
		remove_control_from_container(EditorPlugin.CONTAINER_SPATIAL_EDITOR_MENU, exporter_button)
